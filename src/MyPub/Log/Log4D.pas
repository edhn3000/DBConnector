...
var
  ...
  LogLog: TLogLog;
  //path for external program
  relativePath: string = '';

implementation

uses
  TDailyRollingFileAppenderUnit;
...
begin
  ...
  if relativePath <> '' then
    ...
    relPath := ExtractFilePath(ParamStr(0));

  FFileName := relPath + Name;
...
{$ENDIF}
  ...
  RegisterAppender(TDailyRollingFileAppender);
...
finalization
  ...
  {$ENDIF}
    ...
    on e: Exception do begin
      OutputDebugString(PWideChar('error in free log4d!' + e.Message))
    end;
  end;
end.
