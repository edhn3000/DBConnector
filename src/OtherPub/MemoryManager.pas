  ...
  try
    ...
    begin
      ...
      try
        if TotalAddrSpace <> 0 then
          ...
          Writeln(OutFile, '地址空间载入 : ': FIELD_WIDTH, 0, '%');
      except 
         Writeln(OutFile, '写“地址空间载入信息”出错！ ');
      end;
    end;
    Writeln(OutFile);
    ...
    begin
      ...
      try
        ...
        if not Assigned(PTypeInfo(Item.ClassInfo)) then
          write(OutFile, Item.ClassName+'不是对象(ClassInfo not Assigned)')
        else if PTypeInfo(Item.ClassInfo).Kind <> tkClass then { type info technique }
          write(OutFile, Item.ClassName+'不是对象(Kind <> tkClass)')
        else
        begin
        ...
        on e: Exception do
          write(OutFile, '不是对象(e.Message=' + e.Message + ')');
      end;
      writeln(OutFile);
    end;
  finally
