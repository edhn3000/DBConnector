...
// unit Base64; : 2004/04/22:panwm: extra
...
procedure Base64Encode(const InBuffer;InSize: Cardinal;var
  OutBuffer);overload;register;
  // decodiert die Base64-Darstellung in einen Buffer
{$IFDEF SpeedDecode}
procedure Base64Decode(const InBuffer;InSize: Cardinal;var
  OutBuffer);overload;register;
{$ENDIF}
{$IFDEF ValidityCheck}
...
procedure Base64Decode(const InText: PAnsiChar;var OutText: PAnsiChar);overload;
  ...
  // codiert einen String in die zugehörige Base64-Darstellung
procedure Base64Encode(const InText: AnsiString;var OutText:
  AnsiString);overload;
  // decodiert die Base64-Darstellung eines Strings in den zugehörigen String
procedure Base64Decode(const InText: AnsiString;var OutText:
  AnsiString);overload;
...
var
  str: AnsiString;
begin
  Base64Encode(AnsiString(InText), str);
  Result := str;
end;
...
asm
  ...
  // mov  EAX, EBX : 2004/04/22:panwm: should be EDX ?
  mov  EAX, EDX
  // load 3
  mov  ECX, $03
  // clear upper 32 bits
